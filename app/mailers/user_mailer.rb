class UserMailer < ActionMailer::Base
  ...
  def new_message_sent_advisee(user)
    @user = user
    mail to: @user.email, subject: "PathAdvisor - New message sent to your Advisor"
  end
  ...
  def new_message_sent_advisor(advisor, message)
    @advisor = advisor
    @message = message
    mail to: @advisor.email, subject: "PathAdvisor - You have a new message from an Advisee"
  end
  ...
  def response_sent_advisee(user, message)
    @user = user
    @message = message
    mail to: @user.email, subject: "PathAdvisor - New response message from your Advisor"
  end

  #send to advisor when he has responded to a message from an advisee
  def response_sent_advisor(advisor)
    @advisor = advisor
    mail to: @advisor.email, subject: "PathAdvisor - You have responded to your advisee's message"
  end
  ...
  def new_advicepost_accepted_advisor(advisor)
    @advisor = advisor
    mail to: @advisor.email, subject: "PathAdvisor - New advice listing accepted"
  end
  ...
  def reminder_message_advisor(advisor, message)
    @advisor = advisor
    @message = message
    mail to: @advisor.email, subject: "PathAdvisor - Reminder to respond to new message from an Advisee"
  end
  
  def canceled_message_advisor(advisor, message)
    @advisor = advisor
    @message = message
    mail to: @advisor.email, subject: "PathAdvisor - Advice message canceled"
  end  
